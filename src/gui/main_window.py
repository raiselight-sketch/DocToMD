...
class MainWindow(tkinterdnd2.TkinterDnD.Tk):
    ...
    def __init__(self):
        ...
        self.output_directory = Path(self.config.get("output_directory"))
    ...
    def _build_ui(self):
        ...
        self.keep_chk.grid(row=0, column=2, sticky="e", padx=(0, 12))

        self.ai_chk = ctk.CTkCheckBox(
            list_hdr, text="AI 스마트 후처리 (Gemma4)",
            variable=self.use_ai,
            font=ctk.CTkFont(size=12), text_color=TEXT_SEC,
            border_color=BORDER, fg_color=SUCCESS, checkmark_color=TEXT_PRI
        )
        self.ai_chk.grid(row=0, column=3, sticky="e")
    ...
    def _select_output(self):
        ...
        if d:
            ...
            self.config.set("output_directory", str(self.output_directory))
    ...
    def _worker(self):
        ...
        for f in files:
            ...
            result = self.converter.convert(f, outdir, use_ai=use_ai)
    ...
    def _done(self, ok: int, fail: int):
        ...
        self._show_done_dialog(ok, fail)

    def _show_done_dialog(self, ok: int, fail: int):
        """변환 완료 커스텀 다이얼로그 (폴더 열기 버튼 포함)."""
        ...
        ctk.CTkLabel(
            dlg,
            text=f"성공  {ok}개    실패  {fail}개",
            font=ctk.CTkFont(size=13), text_color=TEXT_SEC
        ).pack()

        ctk.CTkLabel(
            dlg,
            text=self._short_path(self.output_directory, 46),
            font=ctk.CTkFont(size=11), text_color=TEXT_SEC
        ).pack(pady=(4, 18))
        ...
        ctk.CTkButton(
            btn_row, text="📂  폴더 열기", width=150, height=36,
            fg_color=ACCENT, hover_color=ACCENT_DARK, corner_radius=10,
            command=lambda: [self._open_output_folder(), dlg.destroy()]
        ).pack(side="left", padx=8)

        ctk.CTkButton(
            btn_row, text="닫기", width=90, height=36,
            fg_color=CARD2, hover_color=BORDER, corner_radius=10,
            command=dlg.destroy
        ).pack(side="left", padx=8)

    def _open_output_folder(self):
        """Finder(Mac) 또는 Explorer(Windows)에서 출력 폴더 열기."""
        self.output_directory.mkdir(parents=True, exist_ok=True)
        if sys.platform == "darwin":
            subprocess.Popen(["open", str(self.output_directory)])
        elif sys.platform == "win32":
            subprocess.Popen(["explorer", str(self.output_directory)])
        else:
